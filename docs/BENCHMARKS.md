...
| 2026-05-14 | H-4    | M2/arm64    | 50          | —    | —            | —   | —          | Baseline puro de managers (sin Electron, sin tabs reales). IM/WM/BM cold-load <1ms cada uno (50id/10ws/500bk); Vault.unlock con 500 accounts ~0.3ms (AES-GCM round-trip único); BackupManager.createSnapshot full-state ~3.5ms (4 archivos, 230kb uncompressed, 95kb compressed). Mide `tests/perf-baseline.smoketest.js` con soft thresholds (IM/WM/BM <250ms, unlock <500ms, snap <1000ms) — tripwires para detectar 10x regresiones futuras. NO mide cold-start de Electron ni materialización de tabs (eso requiere 1.3-MCP). |
