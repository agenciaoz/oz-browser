// OZ Browser — Bulk action linkedin_post smoke test (v2 Publishing Etapa 6).
//
// Drives buildLinkedinPostAction with a fake BrowserWindow whose executeJavaScript
...
const { buildLinkedinPostAction } = require('../browser/bulk-actions-linkedin-post')
...
function buildResponder(phase = 'normal') {
  return function respond(script) {
    ...
    if (script.includes('Share dialog editor gone')) return true
...
function makeAction(phase, im) {
  return buildLinkedinPostAction({
    identityManager: im,
    electron: {
...
async function main() {
  const im = fakeIM()
  const meta = buildLinkedinPostAction({
    identityManager: im,
    electron: { BrowserWindow: FakeBrowserWindow },
  })

  section('metadata')
  ok('id=linkedin_post', meta.id === 'linkedin_post')
  ok('platform=linkedin.com', meta.platform === 'linkedin.com')
  ok('schema requires text', meta.paramsSchema.required.includes('text'))
  ok('text max 3000', meta.paramsSchema.properties.text.maxLength === 3000)
  ...
  console.log(`\nbulk-actions-linkedin-post: ${passed} passed, ${failed} failed`)
